...
##
cd /var/tmp
git clone https://gitlab.analyticsvidhya.com/edx/configuration
cd configuration
...
##
cd /var/tmp/configuration/playbooks && sudo -E ansible-playbook -c local ./av_dbs.yml -i "localhost," $EXTRA_VARS "$@"
ansible_status=$?
